    ...
    <c:otherwise>
        <c:if test = "${loginedUser.trajetId != 0}">
           <b>Trajet en cours </b><br>
            Station de départ :  ${stations.get(loginedUser.vehicule.getStation_id()).getAdresse()}
            emplacement no :  ${emplacements.get(loginedUser.vehicule.getEmplacement_id()).getId()} <br>
            Station d'arrivé : ${stations.get(loginedUser.trajet.getDestination_station_id()).getAdresse()}
            emplacement no :  ${emplacements.get(loginedUser.trajet.getDestination_emplacement_id()).getId()} <br>
        </c:if>
